...
sbt "runMain learn_fpga_SpinalHDL.MyTopLevelVerilog"

// To generate the VHDL from the example
sbt "runMain learn_fpga_SpinalHDL.MyTopLevelVhdl"

// To run the testbench
sbt "runMain learn_fpga_SpinalHDL.MyTopLevelSim"
```

* The example hardware description is into `hw/spinal/learn_fpga_SpinalHDL/MyTopLevel.scala`
* The testbench is into `hw/spinal/learn_fpga_SpinalHDL/MyTopLevelSim.scala`
...
You might want to change the project name, which is currently `learn_fpga_SpinalHDL`. To do so (let's say your actual project name is `myproject`; it must be all lowercase with no separators):

* Update `build.sbt` and/or `build.sc` by replacing `learn_fpga_SpinalHDL` by the name of your project `myproject` (1 occurrence in each file). The better is to replace in both (it will always work), but in some contexts you can keep only one of these two files:
...
* Put all your scala files into `hw/spinal/myproject/` (remove the unused `hw/spinal/learn_fpga_SpinalHDL/` folder)
...
```sh
// To generate the Verilog from the example
mill learn_fpga_SpinalHDL.runMain learn_fpga_SpinalHDL.MyTopLevelVerilog

// To generate the VHDL from the example
mill learn_fpga_SpinalHDL.runMain learn_fpga_SpinalHDL.MyTopLevelVhdl

// To run the testbench
mill learn_fpga_SpinalHDL.runMain learn_fpga_SpinalHDL.MyTopLevelSim
```
